...
def _validate_pivot_table(data):
    """Validate a pivot table creation action."""
    out = {
        "action": "create_pivot_table",
        "summary": str(data.get("summary", "Crear tabla dinámica")),
    }

    # Validate source_range
    source_range = str(data.get("source_range", "")).strip()
    if not source_range:
        raise ValueError("source_range es requerido para tabla dinámica")
    if not re.match(r'^[A-Z]+\d+:[A-Z]+\d+$', source_range, re.I):
        raise ValueError("source_range debe ser un rango válido (ej: A1:E50)")
    out["source_range"] = source_range

    # Validate row_fields
    row_fields = data.get("row_fields", [])
    if not isinstance(row_fields, list):
        raise ValueError("row_fields debe ser una lista")
    out["row_fields"] = [str(f) for f in row_fields if f]

    # Validate column_fields
    column_fields = data.get("column_fields", [])
    if not isinstance(column_fields, list):
        raise ValueError("column_fields debe ser una lista")
    out["column_fields"] = [str(f) for f in column_fields if f]

    # Validate data_fields
    data_fields = data.get("data_fields", [])
    if not isinstance(data_fields, list):
        raise ValueError("data_fields debe ser una lista")
    if not data_fields:
        raise ValueError("data_fields es requerido con al menos un campo")

    valid_funcs = {"sum", "count", "average", "max", "min"}
    validated_data_fields = []
    for df in data_fields:
        if not isinstance(df, dict):
            raise ValueError("Cada data_field debe ser un objeto")
        field = str(df.get("field", "")).strip()
        if not field:
            raise ValueError("data_field.field es requerido")
        func = str(df.get("function", "sum")).lower()
        if func not in valid_funcs:
            raise ValueError(f"Función '{func}' no válida. Use: {', '.join(valid_funcs)}")
        name = str(df.get("name", field)).strip()
        validated_data_fields.append({"field": field, "function": func, "name": name})
    out["data_fields"] = validated_data_fields

    # Validate dest_cell (optional)
    dest_cell = str(data.get("dest_cell", "A1")).strip().upper()
    if dest_cell and not re.match(r'^[A-Z]+\d+$', dest_cell):
        raise ValueError("dest_cell debe ser una celda válida (ej: G1)")
